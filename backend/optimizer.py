...
def candidate_plans(race_laps: int, compounds: List[str]) -> List[Dict[str, Any]]:
    ...
    if "SOFT" in compounds:
        cands.append({"compounds": ["SOFT"], "pit_laps": []})
    if "MEDIUM" in compounds:
        cands.append({"compounds": ["MEDIUM"], "pit_laps": []})
    if "HARD" in compounds:
        cands.append({"compounds": ["HARD"], "pit_laps": []})
    ...
    p1c2 = int(0.33 * race_laps)
    p2c2 = int(0.66 * race_laps)
    p1c3 = int(0.25 * race_laps)
    p2c3 = int(0.50 * race_laps)
    p3c3 = int(0.75 * race_laps)
...
def plan_to_strategy(plan: Dict[str, Any], race_laps: int) -> List[tuple]:
    ...
    lengths = [cuts[0]] + [cuts[i] - cuts[i - 1] for i in range(1, len(cuts))]
    lengths.append(race_laps - cuts[-1])
...
def evaluate_plans(
    ...
    return {
        ...
        # backward-compatible fields expected by app.py
        "best_strategy": best["strategy"] if best else None,
        "best_time": best["total_race_time"] if best else None,
    }
